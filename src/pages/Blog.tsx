        ...
        <div className="d-flex flex-wrap justify-content-center gap-2 gap-md-3 mb-5">
          {categories.map((cat) => (
            <button
              ...
              className={`px-3 py-2 blog-category-btn ${selectedCategory === cat ? 'blog-category-btn--active' : ''}`}
            >
              {cat}
            </button>
          ))}
          ...
          <AnimatePresence mode="popLayout">
            {filteredPosts.map((post, idx) => (
              <div className="col-12 col-md-6 col-lg-4" key={post.id}>
                <motion.article 
                  className="rounded-1 overflow-hidden h-100 d-flex flex-column blog-article-card"
                  whileHover={{ y: -6 }}
                ...
                >
                  <div className="position-relative overflow-hidden blog-card-img-container">
                    <img src={post.img} className="w-100 h-100 blog-card-img" alt={post.title} />
                    <span className="position-absolute top-3 start-3 px-2 py-1 blog-card-badge">
                      {post.category}
                    </span>
                  </div>
                  <div className="p-4 flex-grow-1 d-flex flex-column justify-content-between">
                    <div>
                      <div className="d-flex align-items-center gap-2 mb-3 blog-card-meta">
                        <Calendar size={14} />
                        <span>{post.date}</span>
                        <span className="ms-2">| 閱讀：{post.readTime}</span>
                      </div>
                      <h3 className="mb-3 blog-card-title">{post.title}</h3>
                      <p className="blog-card-summary">{post.summary}</p>
                    </div>
                    <div className="pt-3 border-top border-white/5">
                      <a href="#" className="d-inline-flex align-items-center gap-2 blog-read-more" onClick={(e) => { e.preventDefault(); alert('文章內容建置中'); }}>
                        閱讀全文 ➔
                      </a>
                    </div>
                  </div>
